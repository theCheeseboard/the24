...
void TimezonesModelDelegate::paint(QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
    ...
    calculator.addRect(offsetRect, [offsetPen, option, painter, index](QRectF bounds) {
        painter->setPen(offsetPen);
        painter->setFont(option.font);
        if (index.data(Qt::UserRole + 3).toBool()) painter->drawText(bounds, Qt::AlignRight | Qt::AlignVCenter, index.data(Qt::UserRole + 1).toString());
    });
    ...
    calculator.addRect(textRect, [painter, mainText, textPen](QRectF bounds) {
        painter->setPen(textPen);
        painter->drawText(bounds, Qt::AlignLeft | Qt::AlignVCenter, mainText);
    });

    QRect zoneName = option.rect;
    zoneName.setLeft(textRect.right() + 6);
    calculator.addRect(zoneName, [offsetPen, index, painter](QRectF bounds) {
        painter->setPen(offsetPen);
        painter->drawText(bounds, Qt::AlignLeft | Qt::AlignVCenter, index.data(Qt::UserRole + 2).toString());
    });
